...
def get_predicted_volumes(pred):
    ...
    return {"anterior": np.sum(pred==1), "posterior": np.sum(pred==2), "total": np.sum(pred==1) + np.sum(pred==2)}
...
def get_series_for_inference(path):
    ...
    dicoms = [pydicom.dcmread(os.path.join(subpath, f)) for subpath in path for f in os.listdir(subpath)]
    ...
    for dicom in dicoms:
        if "HippoCrop" in dicom.SeriesDescription:
...
if __name__ == "__main__":
    ...
    study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)
