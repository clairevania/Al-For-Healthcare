...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    df = pd.merge(df, ndc_df[['Non-proprietary Name', 'NDC_Code']], how="left", left_on='ndc_code', right_on='NDC_Code')
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    validation = df[df[patient_key].isin(unique_values[sample_size_1:sample_size_2])].reset_index(drop=True)
    test = df[df[patient_key].isin(unique_values[sample_size_2:])].reset_index(drop=True)
